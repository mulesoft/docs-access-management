...
Use business groups to organize resources like applications and APIs into self-contained groups within Anypoint Platform. Business groups provide granular control over user access to those resources.
...
Access to resources is determined by assigned user roles and permissions. For example, one user can manage API alerts, while another can only view them.
