...
You may need to redistribute your production, sandbox, and design vCores based on deployment of apps in your business groups.
You can distribute vCores from your master organizations to business groups and their child business groups.
Note that a child business group can only receive resources that are available in the parent business group. For example, a child business group cannot receive sandbox vCores if the parent business group does not have any sandbox vCores available. 
When modifying the business group's resources, if a resource is unavailable, it does not appear in the *Business Group Info* window.
