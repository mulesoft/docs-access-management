...
The *Business Group Info* window includes a resource counter showing a minimum value that equals the amount of the resource that is reassigned to child business groups plus the amount of the resource currently in use by the business group. Unavailable resources are not represented in the window.
