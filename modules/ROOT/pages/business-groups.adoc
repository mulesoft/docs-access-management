...
If you need to redistribute resources (such as vCores, VPCs, load balancers, and so on), you can redistribute resources between business groups at the business group level only (not at the root organization level). 

The business group hierarchy prevents a child business group from consuming resources from either its parent group or the root organization unless an organization administrator redistributes those resources. Resource distribution proceeds from root to parent to child. For example, you can't redistribute sandbox VCores to a child business group if the parent business group doesn't have any sandbox vCores available, even if the root organization has sandbox vCores available.

Additionally, the root organization or parent business group can't reclaim resources that are allocated to a business group that is lower in the hierarchy.
