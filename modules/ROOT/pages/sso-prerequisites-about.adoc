...
* If you delete a SAML identity provider and recreate one with the exact same configuration, it is considered a new identity provider. The users logging into Anypoint Platform using this configuration will be associated with the new identity provider. In case there's an existing user associated with the old/deleted SAML identity provider, duplicate users are seen as they are associated with different identity providers. 
