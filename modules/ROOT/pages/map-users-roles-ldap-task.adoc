...
After you perform this mapping, users in an organization can sign in to Anypoint Platform using the same organizational credentials and access permissions that an organization maintains using SAML, OpenID Connect (OIDC), or LDAP. +
...
. Obtain a response to the `UserInfo` or `IdToken` endpoints with a group claim from your external identity provider.
. In Anypoint Platform, navigate to Access Management.
...
The following examples show responses `UserInfo` or `IdToken` endpoints that contain group claims from different external identity providers.
