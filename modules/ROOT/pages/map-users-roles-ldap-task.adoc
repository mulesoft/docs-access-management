...
External identity providers format group claims in different ways.
Anypoint Platform checks the `User Info` response for the groups claim. If it is not found, it then checks the `Token` response.

You can use any identity providers that return group claims on the `User Info` or `Token` reponses.

=== Map OIDC Group Users to Roles

. Obtain a response to the `UserInfo` or `IdToken` endpoints that contains a group claim from your external identity provider.
. In Anypoint Platform, navigate to Access Management.
. Click *Identity Providers*.
. From *Identity Providers*, select OIDC.
. If your IdP requires an additional OIDC scope to authorize access to groups claims, it can be specified in the *Group Scope* field.
. Verify that the JSONata query in the *Group Attribute JSONata Expression* yields an array of group names from either the User Info or Token response
. Click *Roles*.
. Click *Add Role* to create a role for each group of users in your organization.
. Specify a role name and description.
. Click *Add Role* to add the role.
...
The following examples show responses from the `User Info` or `Token` endpoints that contain group claims from different external identity providers.
