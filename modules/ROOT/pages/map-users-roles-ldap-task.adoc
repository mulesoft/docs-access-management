...
Organizations that use single sign-on and require their users to obtain permissions from multiple groups can configure group mappings using their identity provider. If your IDP provides each group separately, you can create a group attribute that contains multiple groups you want to use. In many cases, you can create an array of groups.
...
To prevent security vulnerabilities, Anypoint Platform requires you to create a single group that encompasses multiple group mappings. For example:
