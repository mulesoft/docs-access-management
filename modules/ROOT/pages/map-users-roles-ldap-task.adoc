...
Anypoint Platform checks the `UserInfo` response for the groups claim. If it is not found, it then checks the `Token` response.

You can use any identity providers that return groups claims on the `UserInfo` or `Token` response.
...
. Verify that the JSONata query in the *Group Attribute JSONata Expression* yields an array of group names from either the `UserInfo` or `Token` response.
. Click *Roles*.
. Click *Add Role* to create a role for each group of users in your organization.
. Specify a role name and description.
. Click *Add Role* to add the role.
...
Given that group claim, the JSONata expression is `$split(custom_attributes.PermissionSets, ',')`.
