...
When you create you client provider following OpenID Connect DCR for Microsoft Entra ID flow, Anypoint populates an introspect url. You can view such urls using `GET /accounts/api/organizations/{orgId}/clientProviders/{clientProviderId}`. When token introspection happens, Anypoint will call `https://graph.microsoft.com/v1.0` with your provided token. As long as the response to such a call is not a failure, the token is determined to be valid.
