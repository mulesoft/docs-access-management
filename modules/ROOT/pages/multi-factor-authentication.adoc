...
When multi-factor authentication is enabled, users must verify their Anypoint Platform credentials using one of the following methods:
...
Built-in authenticator:: Registers a physical authentication device, such as Touch ID, to verify your account. // what happens if they lose this device, and it's their only MFA method?...
