...
MFA is enabled by default for direct signins and can’t be turned off at the organization level. However, a few service account types don’t require MFA. An organization administrator should waive MFA for these accounts so that existing integrations can continue without interruption.

For SSO users, set up MFA through the identity provider. Since MFA is managed on the IdP side, for SSO users, `n/a` is shown for multi-factor authentication in their Settings.
...
. Select your Anypoint identity provider.
