...
The Connected Apps feature provides a framework that enables an external application to integrate with Anypoint Platform using https://anypoint.mulesoft.com/exchange/portals/anypoint-platform/[APIs] via standard protocols, such as SAML, OAuth 2.0, and OpenID Connect.
...
Actions taken by connected apps are auditable, and users can also revoke access at any time.
...
* An xref:connected-apps-end-users.adoc#end-users[end user] can authorize apps to access particular information, such as viewing assets in Exchange.
