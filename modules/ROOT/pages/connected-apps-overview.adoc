...
Client::
An application that makes requests for data on behalf of a user or service with authorization.

Authorization server::
The server that gives access tokens to the client once the server does authorization.

Resource server::
The system that contains the requested data.

Access token:: 
A token needed to access data from the resource server. 

Redirect URI::
The destination the authorization server sends the user or server once the application is authorized.

There are many types of OAuth 2.0 flows; the Connected Apps feature most commonly uses the following:
