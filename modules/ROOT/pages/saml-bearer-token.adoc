...
+
[NOTE]
This method works for IdP-Initiated SSO only as SP-Initiated SSO does not support unsolicited assertions. 
+
...
If the SAML response was received from a HAR file via a text editor, the string is URL encoded. Please use a URL decoder such as urldecoder.org to decode the string first and try again.
+
----
curl -X POST \
