...
If a client exceeds the rate limit for a given control plane, the Audit Log Query Endpoint will return a `503 Service Unavailable` status code until the minute expires. During this time, the service will not be available to the client.
