...
To gather data, you can initiate query loops and use cursors to navigate paginated audit logs. To avoid performance degradation, you can stop the query loop using the `total` by using the value returned by `IncludeTotal`.

The following example shows a series of calls in a query loop:

. The first call does not have a cursor or request for totals. The cursor is paginated: +
[source,console]
----
...
. The third call is the final call in a loop. The response contains no data because is not a cursor, and there are no more logs to be returned.
...
. The final call shows how to use a cursor and request the total: +
