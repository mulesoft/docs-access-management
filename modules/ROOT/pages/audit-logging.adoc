...
To query audit logs, the current https://anypoint.mulesoft.com/exchange/portals/anypoint-platform/f1e97bc6-315a-4490-82a7-23abe036327a.anypoint-platform/audit-log-query-api/minor/1.0/console/method/%23245/ [query api] will continue to work.
Pagination in that api is via offset in the query payload, and the response returns the total number of records.


We are adding a new pagination using cursor and you can initiate query loops by using cursors. This cursor paging can be used by using the `cursorPagination` query parameter and setting it to true.
When using cursor paging, each response will return a cursor. This cursor corresponds to the last data entry in the response data set. So the next request in query loop should include the cursor value that was returned in the previous response, and so on.
When the data set is empty in response, then you have reached the end of query loop, and there is no more data to query for that time window. So, that is the indicator the stop the query loop. When the data set is empty in the response, there will be no cursor returned.
Total is not returned by default when using cursor pagination, as the query loop is not dependent on offset and number of total rows.
When using cursor paging, you can opt in to getting total by using the `includeTotal` query parameter and setting it to true.

When using `cursorPagination` query param as true, if you send `offset` and `cursor` in the query body, we’ll give preference to cursor and ignore the offset.
...
. The first call does not have a cursor in the body; And it does not request for total. The response returns the data and a: 
+
[source,console]
----
...
. The third call is the final call in a loop. There are no more logs to be returned, and so the response contains no data.  Response doesn't have a cursor too as there are no more logs to be returned.
+
[source,console]
----
POST https://base/audit/v2/organizations/<orgId>/query?cursorPagination=true
