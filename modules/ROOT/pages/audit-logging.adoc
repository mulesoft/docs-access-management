...
`Login` and `Logout` actions are logged only at the organization level. Additionally, `Login failure` is logged only when a user enters a password incorrectly for an existing user account. 
