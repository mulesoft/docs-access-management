...
Properties: Application metadata +
Note: Actions performed by MuleSoft support or engineering teams are logged with the User Name `Anypoint Staff`.
|Trigger thread dump
|Application
|AppID
|N/A
...
When you use cursor pagination, each response returns a cursor. The cursor corresponds to the last data entry in the response dataset. The subsequent request in the query loop should use the cursor value that was returned in the previous response.
