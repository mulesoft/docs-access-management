...
Users belonging to the *Organization Administration* role or the *Audit Log Viewer* role on the Anypoint Platform have access to both the UI and the Query API. The audit log service is business group aware, which means you see only logs that are relevant to your own business group. 

The audit log UI is embedded in Access Manager. If you are not an *Organization Administrator*, but you have the *Audit Log Viewer* role, you can see only the audit log user interface under Access Manager.
...
[NOTE]
--
You may occasionally see an *Anypoint Staff* log entry. This log entry is made when the audit action was performed by a user who does not belong to your master organization. +
These actions include when: 

* An internal or system processes is performed
* The SalesOps staff changes entitlements for your organization 
* An action is taken using a bearer token associated with a MuleSoft employee account (for example, during troubleshooting)
--
