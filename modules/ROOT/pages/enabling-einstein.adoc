...
When Einstein is enabled in Anypoint Platform, any Anypoint Platform feature running in that organization is able to use Einstein. The Salesforce administrator can't specify which features are enabled for Einstein, for example, IDP can use Einstein but Anypoint Code Builder can't. 
...
+
Each Salesforce organization has a list of enabled and disabled capabilities.
...
* You can't use the Einstein capabilities that come with a connected Salesforce organization.
...
To enable Einstein for users in your organization, you must assign the required permissions manually. 
