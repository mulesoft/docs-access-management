...
Anypoint Platform supports only PingFederate versions 6 through 8. For version 9 and plus, we recommend setting up the Ping Federate client provider as an OIDC provider following xref:configure-client-management-openid-task.adoc[Configure OpenID Connect Client Management]
