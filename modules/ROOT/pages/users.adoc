...
Control who has access to your Anypoint Platform organization by managing user accounts. As an organization administrator, you can invite users, assign permissions, update user settings, enable and disable users, and remove users when needed.

== Before You Begin

Make sure that you have the required permissions:

* Organization Administrator permission (required for all user management tasks)
* API Version Owner permission (required only if you manage user permissions for your API version)
...
. Optionally, select a xref:teams.adoc[team] or xref:roles.adoc[role] to assign permissions to these users.
. Click *Send Invitation*.

The users you specify receive email invitations to join your organization. The email link expires in one week. When they click the link, they are presented with a sign-up form that already has the *Company* field completed with the name of your organization.
...
* Click the username in the *Username* column.
...
Organization administrators manage user access using xref:teams.adoc[teams] or xref:roles.adoc[roles], or by granting permissions to users individually. Teams and roles functionality are mutually exclusive.
...
You can assign permissions in these areas:

* *API permissions*: Grant access to specific APIs by selecting the API name, version, and permission level.
* *Runtime Manager permissions*: Grant access to Runtime Manager environments by selecting the environment and permission level.
* *Teams*: Add users to teams with predefined permission sets.
* *Roles*: Assign predefined roles that bundle multiple permissions. See xref:roles.adoc[Roles] for descriptions of default roles.
...
+
If environment-level permissions are available, select which environments to apply the permission to. 
. Click *Next*.
