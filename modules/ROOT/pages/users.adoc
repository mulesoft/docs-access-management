...
* *Disable*: Disables the user in the selected Organization / Business Group. They will no longer be able to log in.
...
. Optionally, select a xref:teams.adoc[team] or xref:roles.adoc[role (deprecated)] to which to assign these users.
...
Organization administrators can manage user access using xref:teams.adoc[teams], xref:roles.adoc[roles] (deprecated), or by granting permissions to users individually.
...
If necessary, you can also remove user permissions, but note that if users have been granted their permissions through an assigned role, you can remove role-based permissions using only the xref:roles.adoc[Roles] functionality. You cannot view or remove role-based permissions in the *Users* page.
