...
[NOTE]
--
This feature is not available in Anypoint Platform Private Cloud Edition, Government Cloud, or for organizations that use external identity providers for single sign-on (SSO) and have xref:organization.adoc#account-creation-changes[disabled account creation]. You must manage users, including invitations for new users, through your third-party IdP or LDAP.
--
