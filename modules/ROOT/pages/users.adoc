...
When you delete a user from your root organization, the user is _soft deleted_, wherein a user can no longer access the root organization or business groups and becomes a dangling Anypoint Platform account unless associated with another organization. Anypoint Platform maintains a record of the user's presence for the purposes of audit logs. 

If an Anypoint Platform user that was removed from an organization tries to log in to their former organization, the login request fails, stating `Your credentials are not valid`. 
...
If a deleted federated user tries to log in, the login request fails, stating `This user is disabled`, but the user still appears in the organization as a disabled account. An organization administrator can re-enable this user. 
