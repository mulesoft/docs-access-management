...
In the Anypoint Platform, users belong to organizations and have sets of permissions. These permissions are assigned using xref:teams.adoc[teams] or xref:roles.adoc[roles], or by xref:users.adoc#grant-user-permissions[granting permissions to users individually].
...
Because _API versions_ and _Products deployment environments_ are grouped under organizations and optionally under business groups, accessing them requires an account with the necessary permissions that belongs to the corresponding organization or business group.
...
After a user belongs to a business group within an organization, the only way to assign entitlements to that same user in a different business group is by assigning them a role within that second business group.
--
