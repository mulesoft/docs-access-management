...
You can redistribute resources (such as vCores, VPCs, load balancers, Small or Large Managed Flex Gateways, and so on) between business groups at the business group level only, not at the root organization level. 
...
Each root business group, child business group, and environment, has its own associated unique client ID and client secret. These are used for authentication by users who are not business group administrators to access assets within a business group. The client ID and password are generated by Anypoint Platform for each environment you create, and they are globally unique.
