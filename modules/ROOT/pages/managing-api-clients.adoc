...
Configure up to 25 external client providers in Anypoint Platform to authorize API client applications. Apply OAuth 2.0 policies to protect your APIs, using your chosen OAuth 2.0 provider. This lets you centralize and scale client authorization across environments.
...
[WARNING]
Before you delete a client provider, ensure that you don't have APIs, assets, or environments that depend on the client provider or you will lose functionality. For example, any APIs that depend on the client provider can no longer authenticate user access.
...
* xref:conf-client-mgmt-openam-task.adoc[Configuring OpenAM Client Providers in Access Management]
