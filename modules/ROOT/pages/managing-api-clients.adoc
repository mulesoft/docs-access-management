...
When you delete a client provider from your master organization, the client provider is no longer available in environments that used it. Also, assets that used the client provider can no longer authenticate users who want to access them.
...
[WARNING]
Prior to deleting a client provider, ensure that you do not have assets or environments that depend on the client provider. For example, any APIs that depend on the client provider no longer have a way to authenticate user access.
