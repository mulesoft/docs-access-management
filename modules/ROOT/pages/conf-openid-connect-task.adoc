...
There are two identity management procedures for client registration in OpenID Connect:
...
Although integration with the aforementioned Identity providers has been officially tested, Anypoint Platform supports the OpenID Connect Protocol. This means that any Identity Provider that supports the protocol can integrate unless they diverge from the http://openid.net/specs/openid-connect-core-1_0.html[specification].
