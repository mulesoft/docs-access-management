...
Note, the dynamically registered application at the identity provider has only default settings. If you want to configure additional functionalities e.g. groups mapping, you will be responsible to update the settings on the provider side. Configuring dynamically registered applications is not currently supported. 
