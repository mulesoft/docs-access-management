...
When https://docs.microsoft.com/en-us/graph/api/application-post-applications?view=graph-rest-1.0&tabs=http[creating a new application with Microsoft Azure AD client provider], one won't be able to specify the supported grant types. Thus, Anypoint Platform doesn't support grant types selection due to Microsoft Azure AD limitations.
