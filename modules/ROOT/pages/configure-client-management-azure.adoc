...
.. *Authorization Header*: The header provides credentials to authenticate with a server. This value is the bearer `${api_token}`, where `api_token` is an API token created through Azure. This header is required if the provider restricts registration requests to authorized clients. 
...
.. *Enable client deletion and updates in IdP*: To use this option, you must also select the *Enable client deletion in Anypoint Platform* option. This option enables you to update and delete external clients in the configured IdP through an outbound call made by Azure DCR Wrapper to `https://graph.microsoft.com/v1.0/applications/{clientId}`. +
  * The `client_id` request parameter that is passed is the same as the `client_id` passed in the `PATCH` request payload.
  * The authorization header comes from the Azure client provider configuration. 
+
