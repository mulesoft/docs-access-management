...
=== OAuth 2.0 Endpoints
Anypoint Platform supports only tokens obtained using Azure AD v2.0 endpoints, also known as Microsoft identity platform endpoints. For example, Anypoint Platform supports the token endpoint: `https://login.microsoftonline.com/{tenantId}/oauth2/v2.0/token`. Below is an example of properties to use when you are using 'Authorization Code' grant type to obtain a token. 
...
Note, Anypoint Platform does not support custom scopes. Please use the default scope for Microsoft Graph default scope. 
