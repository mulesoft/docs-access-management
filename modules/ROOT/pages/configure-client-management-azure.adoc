...
Initial secrets expiration is set to 6 months per Microsoft's recommendation. You can reset client secrets via Anypoint Platform. Anypoint Platform creates new secrets on your behalf but does not support removing the old secrets from your Microsoft account on your behalf. However, you can still remove the old secrets in Microsoft accounts.
