...
Anypoint Platform provides support for Microsoft Identity Platform access tokens, Azure AD v2.0 endpoints, grant types, and client secrets.
...
For a list of supported OAuth 2.0 authorization code grant types, see the https://docs.microsoft.com/en-us/azure/active-directory/develop/v2-oauth2-auth-code-flow[Microsoft identity platform documentation].

When https://docs.microsoft.com/en-us/graph/api/application-post-applications?view=graph-rest-1.0&tabs=http[creating a new application with Microsoft Azure AD client provider], you can't specify the supported OAuth 2.0 authorization code grant types. Because of this limitation on the Microsoft Azure AD side, Anypoint Platform does not support selecting grant types.


[[client-secrets]]
=== Client Secrets

Client secrets (application password) use a string value in the Azure application to authenticate instead of a certificate for identity. 

Client secrets are initially set to expire six months from the date of creation per Microsoft's security recommendations. You can reset the expiration date for client secrets in Anypoint Platform. The lifetime limit for a client secret is 24 months and can't be extended beyond that. 

You can also create new client secrets in Anypoint Platform, but you can't remove old or expired secrets from your Microsoft account from Anypoint Platform. You must xref:https://learn.microsoft.com/en-us/partner-center/marketplace-offers/create-or-update-client-ids-and-secrets#to-delete-a-client-secret[delete old and expired secrets] in your Microsoft account. It is best practice to remove expired secrets promptly. 
