...
When you attempt to log in using SSO and there are additional user profiles that share the same email address but use different identity providers (IdPs) to log in, you are prompted to link the profiles. You cannot link profiles that use the same IdP.
...
. In the *Link Multiple Accounts* section, click *Required* so that users must link their user profiles when logging in to Anypoint Platform. 
