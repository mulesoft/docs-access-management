...
This helps to ensure secure credentials and to maintain organizational teams or roles for accessing privileged information.
...
You can map SSO users to teams by including the group names or SAML assertions.
...
+
* OIDC: The groups claims values that come from the `UserInfo` or `Token` endpoint responses +
If your IdP requires an additional OIDC scope to authorize access to groups claims, enter it in the *Group Scope* field, and verify that the https://jsonata.org[JSONata] query in *Group Attribute JSONata Expression* yields an array of group names from either the `UserInfo` or `Token` endpoint response.
+
* LDAP: The string from your SAML assertion AttributeValue
