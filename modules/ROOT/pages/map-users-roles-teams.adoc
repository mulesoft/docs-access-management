...
Map the users in a federated organization's group to a team or role to ensure secure credentials and to maintain organizational structure for accessing privileged information. Users in an organization can then sign in to Anypoint Platform using the same organizational credentials and access permissions that an organization maintains using SAML, OpenID Connect (OIDC), or LDAP. To do this, your Anypoint Platform organization must use an external identity provider, such as PingFederate.
...
To prevent security vulnerabilities, you must create an attribute that encompasses multiple group mappings. 
