...
=== API Access to Team Group Mappings

To access team external IdP group mappings using the Access Management API (for example, `GET /organizations/{orgId}/teams/{teamId}/groupmappings`), have one of these permissions:

* *Access Controls Viewer* (`view:access_controls`) - View team group mappings via API
* *Access Controls Admin* (`admin:access_controls`) - Create, update, or delete team group mappings via API
