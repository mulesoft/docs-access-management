...
Anypoint Platform requires different information from your identity provider (IdP) based on whether you use SAML, OpenID Connect, or LDAP. User mappings apply only to the IdP you configured them for. Performing these steps requires you to move back and forth between applications and the identity provider application to copy or add information.
...
* Ensure the user or identity of the group of users are created in the identity provider.
