...
* If you create a new environment using the name of an existing environment, the newly created environment name has `-<n>` appended to it, where `<n>` is an incremental number.
