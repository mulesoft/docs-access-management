= Anypoint Platform Environments
:page-aliases: managing-cloudhub-environments.adoc

Anypoint Platform lets you create and manage separate deployment environments for APIs and applications. Environments are independent from each other, so you can test applications under the same conditions as your production environment.
...
[NOTE]
--
To change the client secret for an environment, see https://help.salesforce.com/s/articleView?id=001115221&type=1[How to set or reset environment client_secret in Anypoint Platform], or contact your customer support representative.
--
...
The update applies only to new APIs in this environment. If you want to change the client provider for an existing API, you can do that by xref:api-manager::edit-api-endpoint-task.adoc[editing the API in API Manager] and selecting the client provider from the *Client provider* dropdown.
