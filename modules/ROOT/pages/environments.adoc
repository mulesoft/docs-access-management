...
. In the menu on the left, click *Environments*.
. Click the name of the environment.
. Click the *Remove* button next to the name of the client provider that you want to remove. If the client provider is default, first specify another client provider as default by clicking *Set as default*.
. Click *Update*.
