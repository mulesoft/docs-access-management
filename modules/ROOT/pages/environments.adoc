...
You can't grant users access to an environment directly. To grant a user access to an environment use role permissions. See xref:roles.adoc#assigning-permissions-to-roles[Assigning Permissions to Roles] for more information.
////
...
+
NOTE: You can't change the environment type after you create your environment.
...
. Click the name of the environment whose client ID and secret you want to view.
...
You can remove a client provider from an environment at any time. However, before removing the default client provider, specify another client provider as default.
