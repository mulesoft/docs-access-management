...
== Updates to a SAML Identity Provider 
Admin users are able to update an existing SAML provider. However, please note that updating `Issuer`, `Audience` will effectively switch to a different IdP, hence causing potential undesired outcomes, e.g user duplication issue. For more info, see https://help.mulesoft.com/s/article/External-identity-migration-How-to-prevent-duplicate-users-being-created.
