= Configuring SAML SSO for Anypoint Platform
:page-aliases: managing-users.adoc
:page-notice-banner-message: Support for SHA-1 SAML configuration ends on March 15, 2026. To maintain access, contact your Identity Provider or organization administrator to update the configuration to SHA-2.

Use SAML 2.0 with your external identity provider (IdP) to authenticate users and enable single sign-on (SSO) to Anypoint Platform. Configure issuer details, sign-on and sign-off URLs, public key, and audience in the Identity Management form, then test the connection.

Configuring SAML for SSO involves:

* Beginning with a configured SAML IdP
...
* *Issuer*
+
The SAML IdP EntityID (Issuer) URI of the identity provider that sends SAML assertions. This must match the `<EntityID>` value in the IdP’s SAML metadata and the Issuer in its SAML assertions.
+
* *Public Key*
+
Public key provided by the IdP, which is used to sign the SAML assertion. It's the "X509Certificate" value in the SAML Response. 
+
* *Audience*
+
...
Before reassigning the primary key, make sure your IdP can access the key that you intend to use.
...
Before you revoke a key, make sure your IdP is not using the key.
...
When creating a new SAML 2.0 configuration, you can upload an XML file containing your identity provider's SAML 2.0 metadata to Anypoint Platform using the *Import IdP Metadata* link. After you upload the file, its values automatically populate the *Identity Management SAML 2.0* form.
...
Verify that your identity configuration is correct before creating the IdP configuration.

[[export-saml-metadata]]
== Export SAML Metadata

If your IdP supports uploading service-provider metadata, click the *Anypoint service provider metadata* link to download your SAML configuration settings in an XML file. You can then upload this file to your IdP to configure Anypoint Platform. You must have a SAML 2.0 identity configuration to export this data.

[[sso-redirect-using-cookies]]
== SSO Redirect Process Using Cookies

Anypoint Platform uses cookies to make sure that SSO users are redirected to the same location from which they initiated their signin.

. When a user signs in to Anypoint Platform, a cookie is created for the signin session: `mulesoft.sess=xxxxx`.
+
The cookie contains the location where the user initiated the signin, such as the Anypoint Platform page where the user began signing in.
...
. The session cookie, `mulesoft.sess=xxxxx`, is passed from the browser, which redirects the user back to the location where they initiated the signin process.
...
[NOTE]
If a user initiates SSO from an Anypoint Exchange public portal, they are directed to Anypoint Exchange after signing in. Returning to the originating public portal after SSO isn't currently supported.
...
Users who don't have the Organization Administrator permission are directed to Anypoint Exchange.

[[updating-saml-identity-provider]]
== Update a SAML Identity Provider

Organization administrators can update existing SAML providers. However, when you update `Issuer` or `Audience`, you effectively switch to a different IdP, which can cause issues such as unwanted user duplication. For more information, see https://help.salesforce.com/s/articleView?id=001116895&type=1[External identity migration - How to prevent duplicate users being created].
