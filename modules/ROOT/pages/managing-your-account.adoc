...
If you are using a restricted environment, such as a virtual machine, or have proxy requirements that prevent you from logging in to Anypoint Platform, add the following URLs to your allowlists so your sessions are not blocked or interrupted by your organization's firewall or proxy:
