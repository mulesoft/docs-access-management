...
[NOTE]
Apps that use `client_credentials` grant type always have the `profile` scope assigned to them implicitly. The data returned by queries using the scope correspond to the user who created the connected app. The duration of the token is 90 minutes.
...
[[application-whitelist-dev]]
== Application Whitelist

Organization Administrators may enable a whitelist to control which apps can access their users' data.
When your app is whitelisted, only the set of scopes associated with the app are reviewed and approved by the administrator.
...
Existing authorizations and new authorizations using the already-whitelisted set of scopes continue to work.
For more information, see xref:connected-apps-org-admin.adoc#application-whitelist-admin[connected apps for administrators].
