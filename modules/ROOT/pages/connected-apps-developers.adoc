...
Anypoint Platform provides a https://anypoint.mulesoft.com/exchange/portals/anypoint-platform/[comprehensive set of APIs] that that enables developers to extend its functionality.
With the Connected Apps feature, developers can delegate access to applications that interact with Anypoint Platform programmatically using APIs.
...
The Connected Apps feature supports OAuth 2.0 and OpenID Connect. As such, developers must ensure their app can request access to user data using tokens.
...
When a developer creates an app, the app includes a client that requests access to user data.
When an end user receives such a request and authorizes the app to access their data, the app receives an _access token_, allowing the app to call Anypoint Platform on the user’s behalf.
...
Tokens that are not refreshed expire after 90 days. If the app is using the authorization code grant type, the `offline_access` scope is required to use refresh tokens after original access tokens expire.
...
For more information about how to embed the button, refer to the https://github.com/anypoint-web-components/anypoint-signin[developer tutorial] and the https://anypoint-web-components.github.io/anypoint-signin/[developer playground].
...
* Actions taken by the app are logged with an application ID and user ID in the audit log so that you can monitor and audit data use.

You must have the organization administrator role to build an application with Connected Apps.

You should also determine the use-case (for example, CI/CD pipeline) and whether the app is internal to your company or is a third-party application that can be distributed outside of your organization.
