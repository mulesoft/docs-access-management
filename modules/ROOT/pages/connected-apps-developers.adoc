...
When you create connected apps, note the following: 
...
* Your organization owns your app. Any organization administrator over the root organization or the business group in which it resides can control it.
...
* You need the Organization Administrator permission at the level you want to create apps and specify access scopes. 
* You must determine whether the app is internal to your company or is a third-party app that can be distributed outside of your organization.
...
Note: Additional information might be requested based on the grant type you select. For more information about grant types, see xref:connected-apps-developers.adoc#grant-types-and-token-flow[Grant types and token flow].
+
.. *Website URL* +
Users can visit this URL to learn more about the app.
+
.. *Redirect URIs* +
Configure which URIs users are directed to after authorization. Type the URL and click *Add* to add it to the list.
+
.. *Audience* +
Configure whether the app is internal to your organization or is a third-party public app that can be distributed outside of your organization.
+
.. *Scopes* +
...
The `refresh_token` grant type is used to grant access to user data when the user isn’t signed in.
...
* `offline_access`: Access to the user's data when they are not signed in.
