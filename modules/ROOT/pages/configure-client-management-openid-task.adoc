...
If the *Issuer* field isn't configured or the provider's discovery endpoint is unavailable, by default the Anypoint Platfrom UI lists the following grant types when registering an API client application in an xref:exchange::index.adoc[Anypoint Exchange API portal]: 
