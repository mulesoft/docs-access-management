...
+
The header to provide credentials to authenticate with a server. This header is required if the provider restricts registration requests to authorized clients. 
+
[%autowidth.spread]
|===
...
[NOTE]
For Okta, the Okta admin mustt assign the dynamically generated clients to a user or a group of users so they can receive access tokens by sending over the Client ID and Client Secret.
