...
In addition to these scopes, if you have configured the Issuer field while setting up the OIDC Dynamic Client registration provider, Anypoint Platform auto-populates the Anypoint Platform UI with all grant types supported by the provider. This includes grant types such as client credentials, password, etc. To verify the grant types your provider supports, check their discovery endpoint using `$ISSUER/.well-known/openid-configuration`.
