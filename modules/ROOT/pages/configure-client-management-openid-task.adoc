...
Configure an external OpenID Connect (OIDC) identity provider to handle client management for your Anypoint Platform APIs. Use providers supporting the OIDC Dynamic Client Registration (DCR) standard, such as Salesforce, Okta, or OpenAM. This allows API portals to dynamically register client applications with your chosen external IdP when access is requested.
...
. After you have the values from your identity provider’s configuration, complete the information for your client provider, enter the information for *Dynamic Client Registration*:
* *Issuer*
+
URL that the OpenID provider asserts is its trusted issuer.
* *Client Registration URL*
+
URL to dynamically register client applications as a client application for your identity provider.
+
...
|===
.. Click *Use OAuth2 private key jwt* to use a private key instead of and authorization header, and provide these values:
* *Token Client ID*
+
Create a JWT (JSON Web Token) and sign it using the private key. Use this as the client assertion when making the `/token` endpoint API call. 
* *Token Client Private Key*
+
This is the secret cryptographic key used by Anypoint Platform to sign the JWT. When the IdP reads the signed JWT and recognizes the signature, the IdP issue a token to Anypoint Platform. Anypoint Platform then uses that token to register the endpoint to create the client.
...
OKTA or another IDP may allow client modification and deletion independently, but this isn't supported on Anypoint Platform. Instead, Anypoint Platform provides and supports hooks so you can implement API calls that are particular to your IDP. 
...
If you configure the optional *Issuer* field while setting up the OIDC Dynamic Client registration provider, Anypoint Platform autopopulates its UI with all of the OIDC grant types supported by the provider, such as `client credentials`, `password`, and more. 
