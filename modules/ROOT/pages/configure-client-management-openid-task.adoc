...
The Access Management API call `PUT /accounts/api/organizations/{orgId}/clientProviders/{clientProviderId}/clients/{clientId}` has an optional `force` parameter that updates Anypoint Platform internal client application records, regardless of whether the update to the external IdP succeeds or fails. This ensures that Anypoint Platform's view of the client application is updated, even if there are issues communicating with the IdP.

Before the introduction of the `force` parameter, when updating an external client application via the Anypoint Platform API, Anypoint Platform first attempted to update the Identity Provider (IdP). Anypoint Platform then updated its internal records only if the IdP update was successful.
