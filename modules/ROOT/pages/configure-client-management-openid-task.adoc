...
* *Token Client ID*
+
This is the JWT (JSON Web Token) that contains information about the authenticated user, used by OIDC to verify that the user is who they claim to be. 
* *Token Client Private Key*
+
This is the secret cryptographic key used by the client to authenticate itself to a server when requesting access tokens.
* *Token Client API Scopes*
+
The client API scopes define the specific permissions granted to an application when requesting access to a resource.
