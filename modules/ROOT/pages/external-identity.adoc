= Configuring Identity Management in Anypoint Platform
:page-aliases: change-id-mgmt-conf-about.adoc, setting-up-external-identity.adoc

As an Anypoint Platform organization administrator, you can configure identity management to enable single sign-on (SSO) for your organization's users. Anypoint Platform supports OpenID Connect for end-user identity verification and SAML 2.0 for web-based authorization, and allows up to 25 external identity providers (IdPs).

Configure identity management using one of these single sign-on standards:
...
Anypoint Platform fully supports these SAML providers:
...
These SAML providers are known to work, but aren't actively tested:
...
Anypoint Platform fully supports these OpenID Connect providers:
