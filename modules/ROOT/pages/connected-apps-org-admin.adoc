...
When an organization administrator leaves the organization or is deleted or disabled, the ownership of the connected apps owned by that user is automatically transferred to the organization owner. When ownership is transferred, it is best practice to manually rotate the client secrets of all the connected apps in the organization for security reasons. For more information about how to rotate client secrets, see xref:connected-apps-developers#modify-the-client-secret-for-a-connected-app[Modify the Client Secret for a Connected App].
