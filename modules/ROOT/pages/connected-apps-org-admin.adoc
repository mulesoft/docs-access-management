...
The `credentialsUpdatedAt` timestamp records when the connected app's client secret was last updated. It updates only when the client secret changes, such as when you reset the secret, but not when you modify other app details like the name or redirect URIs.

[NOTE]
--
This field was added without a historical backfill, so the timestamp doesn't reflect secret changes made before June 4, 2026. Connected apps whose secret hasn't changed since then return the app's creation date as a fallback instead of an actual rotation timestamp.
--
