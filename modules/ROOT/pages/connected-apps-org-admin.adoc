...
== Master organization and business groups

By default, connected apps are configured at the master organization level, not at the business group (sub-organization) levels.

If a connected app is created to act on its own behalf(client credentials), after the scopes scopes are chosen, you could choose which master ogranization/Business Groups to enable it.

If a connected app is created to act on behalf of a user, what the connected app can do is based on the scopes and what organization/business groups this user has access to.
