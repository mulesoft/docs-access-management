...
+
Specify whether SSO can be initiated by the Anypoint Platform, your identity provider (e.g., Okta), or both. 
...
- The *Both* option means either Anypoint Platform or your external identity provider can initiate SSO. 

The default value for this setting is *Both (recommended)* for newly configured identity provider configurations. For existing identity provider configurations, the default setting is *Identity Provider Only*.
+
