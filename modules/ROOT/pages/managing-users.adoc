...
== SSO Redirects using Cookies

Anypoint Platform uses cookies to ensure SSO users are redirected to the same location from which they initiated their log in.
...
After the user is authenticated using SSO, the provider directs the user back to Anypoint Platform with the `receive-id` SAML Response.
The session cookie, `mulesoft.sess=xxxxx`, is passed from the browser, which re-directs the user back to the location from which they initiated the login process.

After the user successfully logs in to Anypoint Platform, the `mulesoft.sess` cookie is reset to default based on the OWASP standard.

In some instances, users are redirected based on their account's permissions:
