...
. In Anypoint Platform, sign in to the master organization as a user with the Organization Administrator permission.
...
+
The default value for this setting for newly configured identity provider configurations is *Both (recommended)*. For existing identity provider configurations, the default setting is *Identity Provider Only*.
+
