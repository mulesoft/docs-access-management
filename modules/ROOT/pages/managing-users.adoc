...
. In Anypoint Platform, sign into the master organization as a user with the *Organization Administrator* permission.
...
+
Specifies which identity provider you use to initiate single sign-on (SSO). The *Service Provider Only* option allows Anypoint Platform to initiate SSO. The *Identity Provider Only* option allows your external identity provider to initiate SSO. The *Both* option means either Anypoint Platform or your external identity provider can initiate SSO. The default value for this setting is *Both* for newly configured identity provider configurations. For existing identity provider configurations, the default setting is *Identity Provider Only*.
+
