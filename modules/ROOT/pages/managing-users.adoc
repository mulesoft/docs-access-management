...
You can import your identity provider's SAML 2.0 metadata to Anypoint Platform. You can use *Import IdP Metadata* to import an XML file that contains a SAML 2.0 configuration from your identity provider. The XML file's values populate the *Identity Management SAML 2.0* form.

When you upload an XML file to import your metadata:

* The file must be smaller than 16 KB.
* The file must contain values that correspond to the fields in the *Identity Provider* page.
* When you upload an XML file that contains more than one identity configuration, Anypoint Platform uses the first available identity provider configuration.
* Anypoint Platform does not check the validity of the values in the XML that you upload. You must ensure your identity configuration is correct before creating the identity provider configuration.
