= Configuring OpenAM Client Providers in Anypoint Platform

Register API clients through OpenAM 14 by configuring an OpenAM client provider in Anypoint Platform. Map OAuth2 authorization, token, validation, and client management endpoints along with admin token credentials to your deployment. Apply the OpenAM OAuth token enforcement policy to APIs through API Manager.
...
* xref:api-manager::mule-oauth-provider-landing-page.adoc[]
* xref:api-manager::openam-oauth-token-enforcement-policy.adoc[Apply the OpenAM OAuth Token Enforcement Policy to Your APIs]
